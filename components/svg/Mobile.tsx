import { FC } from 'react'

const Mobile: FC<{ className: string }> = ({ className }) => {
	return (
		<>
			<svg
				className={className}
				version="1.1"
				id="Capa_1"
				xmlns="http://www.w3.org/2000/svg"
				x="0px"
				y="0px"
				viewBox="0 0 97.92 97.92"
			>
				<g>
					<path
						style={{ fill: '#555A53' }}
						d="M76.22,92.766c0,2.848-2.308,5.154-5.154,5.154H26.853c-2.847,0-5.153-2.307-5.153-5.154V5.154
		C21.7,2.307,24.006,0,26.853,0h44.213c2.847,0,5.154,2.307,5.154,5.154V92.766z"
					/>
					<path
						style={{ opacity: '0.15' }}
						d="M71.066,92.714H26.853c-2.847,0-5.153-2.307-5.153-5.154v5.206c0,2.848,2.307,5.154,5.153,5.154
		h44.213c2.847,0,5.154-2.307,5.154-5.154V87.56C76.22,90.407,73.913,92.714,71.066,92.714z"
					/>
					<rect x="26.754" y="12.693" style={{ fill: '#54A2BD' }} width="44.408" height="68.768" />
					<path
						style={{ opacity: '0.2', fill: '#FFFFFF' }}
						d="M38.436,12.693H26.754v11.682C30.648,20.481,34.542,16.587,38.436,12.693z"
					/>
					<path
						style={{ opacity: '0.2', fill: '#FFFFFF' }}
						d="M71.163,17.927v-5.233H59.167L26.754,45.105v17.229
		C41.557,47.532,56.36,32.729,71.163,17.927z"
					/>
					<path
						style={{ opacity: '0.15' }}
						d="M26.754,12.693v68.768h44.408V12.693H26.754z M69.005,78.12H28.912V14.241h40.094V78.12z"
					/>
					<path
						style={{ opacity: '0.15' }}
						d="M60.668,7.984c0,0.809-0.656,1.463-1.466,1.463H38.714c-0.809,0-1.463-0.654-1.463-1.463l0,0
		c0-0.81,0.654-1.466,1.463-1.466h20.488C60.012,6.518,60.668,7.174,60.668,7.984L60.668,7.984z"
					/>
					<path
						style={{ fill: '#A6B3A4' }}
						d="M60.668,7.099c0,0.809-0.656,1.463-1.466,1.463H38.714c-0.809,0-1.463-0.654-1.463-1.463l0,0
		c0-0.809,0.654-1.466,1.463-1.466h20.488C60.012,5.633,60.668,6.289,60.668,7.099L60.668,7.099z"
					/>
					<circle style={{ fill: '#A6B3A4' }} cx="48.959" cy="89.834" r="4.593" />
					<path
						style={{ opacity: '0.15' }}
						d="M48.959,92.153c-2.142,0-3.927-1.473-4.435-3.456c-0.094,0.365-0.159,0.742-0.159,1.137
		c0,2.535,2.058,4.592,4.594,4.592c2.537,0,4.593-2.057,4.593-4.592c0-0.395-0.065-0.771-0.158-1.137
		C52.886,90.681,51.102,92.153,48.959,92.153z"
					/>
				</g>
				<g></g>
				<g></g>
				<g></g>
				<g></g>
				<g></g>
				<g></g>
				<g></g>
				<g></g>
				<g></g>
				<g></g>
				<g></g>
				<g></g>
				<g></g>
				<g></g>
				<g></g>
			</svg>
		</>
	)
}
export default Mobile
