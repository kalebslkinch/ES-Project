...
const Billing = ({}) => {
	...
	const quantity: number[] = getCart.map(data => data.quantity)
	const IDs: number[] = sortedCookies.map(data => data.id).sort()

	const matchingIDs = getProductsAvaialable.allProductsAvailable.data.filter(data => IDs.includes(data._id))
	...
	const handleCheckout = (e): void => {
		...
		// Data Encryption
		let encryptedData = CryptoJS.AES.encrypt(JSON.stringify(data), `${process.env.SECRET_KEY}`).toString()
	...
	if (getCart.length > 0) {
		...
		if (succeeded) {
			...
			if (parseInt(cookie.get('done'), 10) <= 0) {
				let add: number = parseInt(cookie.get('done'), 10)
				addCustomersOrders({
					variables: {
						data: {
							customerID: user ? user.id : '',
