...
const MyProfile = ({ user }) => {
  ...
  if (user) {
    ...
    const newOrdersLengthAmount = data.allMembersOrders.data.filter(
      (data) => data.completed === false
    ).length;
    const shippedOrdersAmount = data.allMembersOrders.data.filter(
          ...
          <div className='-mbg-gray-800 mx-auto mt-10 w-[600px] animate-fadein overflow-hidden rounded-lg bg-white   md:my-auto md:w-[500px]'>
            <TransitionScaleIn>
              ...
              <Row className=' items-center bg-gray-900 py-2'>
                ...
                <h1 className='mx-2 text-lg font-semibold text-white'>
                  Profile
                </h1>
              </Row>

              <div className='px-6 py-3'>
                <h1 className='-mtext-white mt-2 pb-1 text-xl  font-semibold text-gray-800'>
                  {user.name}
                </h1>
